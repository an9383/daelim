	...
	<header class="page-title-bar row">
		<nav aria-label="breadcrumb" class="breadcrumb-padding">
			<ol class="breadcrumb">
				...
				<li class="breadcrumb-item active">반출등록(봉제)</li>
			</ol>
		</nav>
	</header>
	<!-- #main============================================================== -->
...
// 			$(this).removeClass('selected');
    	...
    	if ( $('input[type="checkbox"][name="childTableCheck"]:checked').length > 0 ) {
    		...
    		$('#outWhsAdmChildTable tbody tr').each(function(index, item) {
    			if ( $(this).find('td input[type="checkbox"][name=childTableCheck]').is(":checked") ) {
    				...
    				rowData.itemSeq = outWhsAdmChildTable.row(this).data().itemSeq;
    				rowData.beforeLocCd = outWhsAdmChildTable.row(this).data().beforeLocCd;
    				rowData.beforeLocNo = outWhsAdmChildTable.row(this).data().beforeLocNo;
    				rowData.toQty = outWhsAdmChildTable.row(this).data().toQty;
	...
	$('#btnSave').on('click', function() {
		if ( itemInsertTable.data().length > 0 ) {
			...
			if(!$.trim($('#kindDealCorpCd').val())){
				toastr.warning('고객사/외주사를 선택해주세요.');
				$('#dealCorpCdBtn').focus();
				check = false;
				return false;
			}
			...
			$('#itemInsertTable tbody tr').each(function(index, item){
				...
				rowData.mainGubun 		= "002"; //부적합관리(봉제) 창고 조회 데이터
				rowData.locCd 			= "018"; //부적합관리(봉제) 창고 조회 데이터
	...
	function dealCorpCdModalShow() {
		...
		$('#btnDealCorpSelect').on('click', function() {
			if ( dealCorpCdTable.rows('.selected').data().length > 0 ) {
				...
				$('#kindDealCorpCd').val($('#dealGubunOption').val()); //고객사: 001, 외주사: 003
	...
	let itemAllPopupTable = $('#itemAllPopupTable').DataTable({
        ...
        scrollY: "30vh",
		ajax : {
			url : '<c:url value="bs/takeOffItemList"/>',
			type : 'GET',
			data : {
				'kindDealCorpCd': function(){ return $('#kindDealCorpCd').val(); },
				'itemCusList'	: function(){ return $('#listDealCorpCd').val(); },
				'mainGubun'		: function(){ return "002"; },
				'toQtyOption' 	: function(){ return "Y"; },
				'tmMon'			: function(){ return SERVER_DATE.substring(0, 7).replace(/-/g,''); },
				'locationCd' 	: function(){ return '017'; },
			},
		},
		columns : [
			{ //체크박스
	...
	$('#btnGetItemList').on('click', function(){
		$('#itemAllTempTable tbody tr').each(function(index, item) {	
			...
			$('#itemInsertTable').DataTable().row.add({
				"kindDealCorpCd": $('#kindDealCorpCd').val()
	...
	//품목조회 팝업창 상단 테이블 행 더블클릭 시 품목추가처럼 행 추가되는 기능
