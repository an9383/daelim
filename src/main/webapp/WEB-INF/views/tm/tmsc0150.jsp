					...
					<table id="barcodeDtlTable" class="table table-bordered table-td-center">
						<thead class="thead-light">
							<tr>
								...
								<th class="text-center" style="min-width: 60px">양품수량</th>
								<th style="min-width: 90px">수입검사일</th>
							</tr>
						</thead>
					</table>
				</div>
...
<script>
	...
	$(document).attr("title","자재입고 승인처리");
	let menuAuth = 'tmsc0150';
		...
		if(techNote == 7) {
			...
			columns : [ 
					...
					{data : 'pairCnt',
						render: function(data, type, row, meta) {	
							if(data!=null){
								return '<input type="text" class="form-control" style="max-width:100%; text-align:right;" name="pairCnt" value="'+addCommas(data)+'" disabled>';			
							} else{
								return '<input type="text" class="form-control" style="max-width:100%; text-align:right;" name="pairCnt" value="'+addCommas(pairCnt)+'" disabled>';	
							}				
			    		}
					}, 
					{data : 'inspectDate',
						render: function(data, type, row, meta) {	
							if(data!=null){
								return '<input type="text" class="form-control" style="max-width:100%; text-align:center;" name="inspectDate" value="'+moment(data).format('YYYY-MM-DD')+'" disabled>';			
							} else{
								return '<input type="text" class="form-control" style="max-width:100%; text-align:center;" name="inspectDate" value="'+moment(inspectDate).format('YYYY-MM-DD')+'" disabled>';	
							}		
						}
					}
			],
			 	...
			 	$.ajax({
	                ...
	                success: function (res) {
	                    ...
	                    if (res.result == 'ok') {
		                    ...
		                    pairCnt = addCommas(data.pairCnt);
	...
	$('#btnApprove').on('click',function() {
		...
		$('#barcodeDtlTable tbody tr').each(function(index, item){
			...
			rowData.apQty= $(this).find('td input[name=pairCnt]').val().replace(/,/g,'');	
