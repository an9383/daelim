	...
	<header class="page-title-bar row">
		<nav aria-label="breadcrumb" class="breadcrumb-padding">
			<ol class="breadcrumb">
				...
				<li class="breadcrumb-item active">현재고현황(봉제)</li>
			</ol>
		</nav>
	</header>
	<!-- #main============================================================== -->
...
<script>
	...
	let currentHref = "tmsc0051";
	let currentPage = $('.' + currentHref).attr('id');
	$(document).attr("title", "현재고현황(봉제)");
...
//  			$(this).removeClass('selected');
	...
	function rowDateColorChange(){
		if(selectSpType=='003' || selectSpType=='004'){	//제품 제외
	...
	let inventoryDtlTable = $('#inventoryDtlTable').DataTable({
		...
		scrollY : "27vh",
		ajax : {
			url : '<c:url value="tm/stockPaymentAdmReadByItem"/>',
			type : 'GET',
			data : {
				'mainGubun' : function(){ return '002'; },
...
<script>
	...
	$('#printBarcode').on('click', function() {
		...
		$('input[name=printList]:checked').each(function(index, item) {
			...
			// 양식에 맞추어 컬럼 집어넣기
			if (selectSpType == '001') { // 제품
				paperSize = "productLabel";
				columnArr = ["차종", "품목번호", "품명", "생산일자", "수량", "작업자", "납품처", "공정검사 확인"];
				
				trPrintArr[0] = subTrData.itemModelNm;
				trPrintArr[1] = itemCd;
				trPrintArr[2] = itemNm;
				trPrintArr[3] = moment(subTrData.inDate).format("YY/MM/DD");
				trPrintArr[4] = parseFloat(subTrData.realQty) + " " + spUnit;
				trPrintArr[5] = workChargrNm;
				trPrintArr[6] = subTrData.itemCusNm;
				trPrintArr[7] = gongjungChk;
				trPrintArr[8] = subTrData.barcodeNo;
				
			} else if (selectSpType == '003') { // 반제품
				paperSize = "productLabel";
				columnArr = ["차종", "품목번호", "품명", "생산일자", "자주검사", "잔량", "작업자", "공정검사 확인"];
