...
@Controller
public class totalAttachAdmController {

	private static final Logger logger = LoggerFactory.getLogger(totalAttachAdmController.class);
	
	@Inject
	private CommonCodeAdmService systemCommonCodeService;
	
	//통합자료실 메인
	@RequestMapping(value = "/tasc0010", method = RequestMethod.GET)
	public String totalAttachAdmMain(Locale locale, Model model) throws Exception {
		logger.info("통합자료실 규격집 메인");
		try {
			Date date = new Date();
