	...
	<header class="page-title-bar row">
		<nav aria-label="breadcrumb" class="breadcrumb-padding">
			<ol class="breadcrumb">
				...
				<li class="breadcrumb-item active">개발관리(봉제)</li>
			</ol>
		</nav>
	</header>
	<!-- #main============================================================== -->
			...
			<div class="right-list right-sidebar" id="myrSidenav" style="width: 49%;">
				...
				<div class="card-header card-tab p-0 mb-2">
					<ul class="nav nav-tabs card-header-tabs m-0">
						...
						<li class="nav-item">
							<a class="nav-link disabled" data-toggle="tab" href="#tab3" id="tab2Nav">BOM등록</a>
						</li>
						<li class="nav-item">
							<a class="nav-link disabled" data-toggle="tab" href="#tab4" id="tab3Nav">관련자료</a>
						</li>
					</ul>
				</div>
				<!--====================TAB-UI=======================-->
					...
					<div id="myTabContent" class="tab-content">
						...
						<div class="tab-pane fade active show" id="tab1">
							...
							<form id="form1" enctype="multipart/form-data">
								<table class="table table-bordered">
									...
									<tr>
										<th>개발단가</th>
										<td><input type="text" class="form-control" style="text-align:right; min-width:100%;" id="devCost" name="devCost" value="0" onkeyup="numberFormat(this, 'float')" disabled></td>
										<th>확정단가</th>
										<td><input type="text" class="form-control" style="text-align:right; min-width:100%;" id="confirCost" name="confirCost" value="0" onkeyup="numberFormat(this, 'float')" disabled></td>
									</tr>
									<tr>
										<th>개발단계</th>
									...
									<tr>
										...
										<td>
											<div class="form-group input-sub m-0" style="min-width:100%;">
												<input class="form-control" type="date" id="startDate" name="startDate" maxlength="10" style="min-width:100%;" disabled />
											</div>
										</td>
										<th>양산일자</th>
										<td>
											<div class="form-group input-sub m-0" style="min-width:100%;">
												<input class="form-control" type="date" id="complDate" name="complDate" maxlength="10" style="min-width:100%;" disabled />
											</div>
										</td>
									</tr>
									<tr>
						...
						<div class="tab-pane fade" id="tab2">
							<div class="card-body col-sm-12 p-1">
								...
								<button type="button" class="btn btn-warning float-left ml-1" id="btnPrcssDel">삭제</button>
...
<script>
	...
	let menuAuth = 'bmsc0190';
	let currentHref = "bmsc0190";
	let currentPage = $('.' + currentHref).attr('id');
	$('#' + currentPage).closest('.has-child', 'li').addClass(
			'has-open has-active');
	$('#' + currentPage).closest('.menu-item').addClass('has-active');
	$(document).attr("title","개발관리(봉제)"); 
	...
	let itemInfoAdmTable = $('#itemInfoAdmTable').DataTable({
		...
		ajax : {
			...
			data : {
				...
				'orderByItemCd' : function(){return "Y"}
			},
		},
		rowId : 'bomCd',
		columns : [ 
...
// 			url = '<c:url value="/bm/prcssBomCreate"/>';
            ...
            success: function (res) {
                ...
                if(res.result == 'ok') {
                    ...
                    for(var i=0;i<data.length;i++) {
            			...
            			itemInfoAdmHtml	+= '			<td class="text-center-td">'+data[i].prcssNm+'</td>';
            			itemInfoAdmHtml	+= '		</tr>';
                    }
                    itemInfoAdmHtml	+= '	</tbody>';
		...
		columns : [
				...
				render : function(data,type,row,meta){
					if(data!=null){
						...
						if(row['itemLvl']=="0"){
							...
							return '<input type="text" class="form-control text-right" name="itemConsumpt" value="'+ouputNumberAutoComma(data)+'" style="border:none;cursor:default;" readonly>';
						}
					}else{
			...
			},
			{
				...
				render : function(data,type,row,meta){
					...
					if(data != null){
						var delHtml = '<div class="ml-1" style="width: 5%;">';
						delHtml += '<button type="button" name="btnPrcssCdDel" class="btn closeBtn p-1" onclick="removeData(\'prcssCd\',\'prcssNm\','+meta.row+')" disabled>';
						delHtml += '<i class="mdi mdi-close"></i></button></div>';
						
						var html = '<div class="input-sub m-0" style="width: 80%;">';
						html += '<input type="text" class="form-control" name="prcssNm" value="'+(row['prcssNm']==null?"":row['prcssNm'])+'" disabled>';
						html += '<input type="hidden" class="form-control" name="prcssCd" value="'+data+'" disabled>';
						html += '<button type="button" name="btnPrcssCd" class="btn btn-primary input-sub-search" onClick="prcssCdClick('+meta.row+')" disabled>';
						...
						html += '<input type="text" class="form-control" name="prcssNm" value="'+(row['prcssNm']==null?"":row['prcssNm'])+'" disabled>';
						html += '<input type="hidden" class="form-control" name="prcssCd" value="'+data+'" disabled>';
						html += '<button type="button" name="btnPrcssCd" class="btn btn-primary input-sub-search" onClick="prcssCdClick('+meta.row+')">';
						html += '<span class="oi oi-magnifying-glass"></span>';
						html += '</button>';
						html += '</div>';
					}	
					...
					*/
					if(data!=null){
						let html = "";
						html += '<input type="hidden" class="form-control route-border" name="prcssCd" value="'+data+'">';
						html += '<input type="text" class="form-control route-border" name="prcssNm" value="'+row['prcssNm']+'" style="cursor: default;" readonly>';
						return html;
					}else{
						let html = "";
						html += '<input type="hidden" class="form-control route-border" name="prcssCd" value="">';
						html += '<input type="text" class="form-control route-border" name="prcssNm" value="" style="cursor: default;"  readonly>';
						return html;
					}
				},
				'className' : 'text-center'
		...
		columns : [
			...
			},
			{
				...
				render : function(data,type,row,meta){
					if(data!=null){
						let html ="";
						html += '<input type="hidden" class="form-control route-border" name="prcssCd" value="'+data+'" disabled>';
						html += '<input type="text" class="form-control route-border" name="prcssNm" value="'+row['prcssNm']+'" disabled>';
						return html;
					}else{
						let html ="";
						html += '<input type="hidden" class="form-control route-border" name="prcssCd" value="" disabled>';
						html += '<input type="text" class="form-control route-border" name="prcssNm" value="" disabled>';
						return html;
					}
				},
				'className' : 'text-center'
	...
	$('#btnRouteEdit').on('click', function() {
		...
		$('button[name=btnPrcssCd]').attr('disabled',false);
	...
	$('#btnDownAdd').on('click',function(){
		if ( tableIdx2 == null ) {
			toastr.warning("하위에 추가 할 항목을 선택해주세요.");
			return false;
		}
...
// 				$("#etc1"+i).attr('name', 'etc1');
	...
	let itemCdPopUpTable2 = $('#itemCdPopUpTable2').DataTable({
		...
		columns : [ 
			...
			{ data : 'itemGubunNm'}, 	// column 7 - 숨김
			{ data : 'itemCd'}, 		// column 8 - 숨김
			{ data : 'itemNm'}, 		// column 9 - 숨김
			{ data : 'itemModelNm'}, 	// column 10 - 숨김
			{ data : 'itemUnitNm'}, 	// column 11 - 숨김
			{ data : 'itemWorkplace'}, 	// column 12 - 숨김
		],
		columnDefs : [ 
	...
	$('#btnGetItem').on('click',function(){
		let check = true;
		let rowCount = itemCdPopUpTempTable.data().count();
		if ( rowCount == 0 ) {
		...
		if (check) {
			let startIdx = tableIdx2 + 1;
			let endIdx = itemMotorEbomTable.data().count();
			let idxArr = [];
			...
			for ( var i=startIdx; i<endIdx; i++ ) {
				...
				$('#itemMotorEbomTempTable').DataTable().row.add({
					...
					'prcssNm'	: $('#itemMotorEbomTable tbody tr').eq(i).find('td input[name=prcssNm]').val(),
				}).draw(false);
			...
			$('#itemCdPopUpTempTable tbody tr').each(function(index, item) {
				...
				$('#itemMotorEbomTable').DataTable().row.add({
					...
					'prcssNm'	: $tr.find('td input[name=itemWorkplace]').val(),
				}).draw(false);
			...
			let endIdx2 = itemMotorEbomTempTable.data().count();
			...
			for(var i=0; i<endIdx2; i++){
				...
				$('#itemMotorEbomTable').DataTable().row.add({
					...
					'prcssNm'	: itemMotorEbomTempTable.row(i).data().prcssNm,
				}).draw(false);
			}
