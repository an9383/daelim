	...
	<header class="page-title-bar row">
		<nav aria-label="breadcrumb" class="breadcrumb-padding">
			<ol class="breadcrumb">
				...
				<li class="breadcrumb-item active">제조리드타임(사출)</li>
			</ol>
		</nav>
	</header>
		...
		<div class="row table-wrap-hid">
			...
			<div class="left-list left-sidebar-fix" id="left-list" style="width: 100%;">
				<div class="card">
					...
					<div class="row mb-2">
						<label class="input-label-sm mr-2">수주일자</label>
...
<script>
	let currentHref = "mmsc0200";
	let currentPage = $('.' + currentHref).attr('id');
	
	$('#'+currentPage).closest('.has-child','li').addClass('has-open has-active');
	$('#'+currentPage).closest('.menu-item').addClass('has-active');    
	$(document).attr("title","제조리드타임"); 
	...
	let table = $('#table').DataTable({
		...
		scrollY : "65vh",
		ajax : {
			url : '<c:url value="mm/leadTimeList"/>',
			type : 'GET',
			data : {
				'regDateFrom' : function() {return regDateFromVal.replace(/-/g,'');},
				'regDateTo' : function() {return regDateToVal.replace(/-/g,'');},
	...
	let html1  = '<div class="row">';
	    html1 += '	<label class="input-label-sm mr-2">수주일자</label>';
