	...
	<header class="page-title-bar row">
		<nav aria-label="breadcrumb" class="breadcrumb-padding">
			<ol class="breadcrumb">
				...
				<li class="breadcrumb-item active">소요량분석(봉제)</li>
			</ol>
		</nav>
	</header>
	<!-- #main============================================================== -->
								...
								<table id="bizOrderConsumptTable" class="table table-bordered">
									...
									<thead>
										<tr>	
											...
											<th class="d-none">확정수량</th>
											<th class="d-none">단가</th>		
											<th class="d-none">금액</th>			
										</tr>
									</thead>
								</table>
							</div>
	...
	<div class="modal modal-alert fade" id="resetModal" data-backdrop="static" tabindex="-1" role="dialog" aria-labelledby="resetModal" aria-hidden="true">
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				...
				<div class="modal-body">
					<h6>소요량분석(봉제) 새로고침/수정 방식을 선택해주세요. </h6>
				</div>
				<div class="modal-footer">
		...
		<div class="modal-dialog modal-xl" role="document">
			<div class="modal-content">
				...
				<div class="modal-body">
					...
					<div class="table-responsive">
						<table id="monSalesPlanTable" class="table table-bordered tr_highlight_row" style="width:100%">
							<colgroup>
								<col width="10%">
								<col width="10%">
								<col width="20%">
							...
							<thead>
								<tr>
									<th class="text-center">
										<input type="checkbox" id="checkAll" name="checkAll" style="width:1.2rem; height:1.2rem;">
									</th>
					...
					<hr class="m-2 d-none">
					<button type="button" class="btn btn-danger d-none" id="btnItemRemove">품목삭제</button>
					...
					<div class="modal-footer">
						<button type="button" class="btn btn-primary" id="btnConsumptAnalz">소요량분석(봉제)</button>
						<button type="button" class="btn btn-secondary" data-dismiss="modal">닫기</button>
					</div>
				</div>
			</div>
...
<script>
	...
	let currentHref = "iosc0290";
	let currentPage = $('.' + currentHref).attr('id');

	$('#' + currentPage).closest('.has-child', 'li').addClass(
			'has-open has-active');
	$('#' + currentPage).closest('.menu-item').addClass('has-active');
	$(document).attr("title", "소요량분석(봉제)");
	...
	var status = "";				//소요량분석(봉제) 목록조회시 조건
	var statusOptionVal = '002'; 		//소요량분석(봉제) 상태값
	var itemSeqVal=null;
	var dealGubun = "";
    ...
    var prodMon = mon;
  	...
  	//공통코드 처리 시작
	let prodGubunArray = new Array();
	<c:forEach items="${prodGubunList}" var="info">
	var baseNm = "${info.baseNm}";
	if ( baseNm.includes('외주') ) {
		prodGubunArray.push("${info.baseCd}");
	}
	</c:forEach>
	//공통코드 처리 끝
	...
	$('#btnRetv').on('click', function() {
		if(!$.trim($('#mrpDateFrom').val())||!$.trim($('#mrpDateTo').val())){
			toastr.warning('날짜를 선택해주세요.');
			if(!$.trim($('#mrpDateFrom').val())){
				$('#mrpDateFrom').focus;
			} else {
				$('#mrpDateTo').focus;
			}
	...
	$('#bizOrderAdmTable tbody').on('click','tr',function() {
		...
		status = 'read';		//소요량클릭시 소요량분석(봉제) 목록조회 조건
		sideView = "edit";
	...
	$('#checkAll').on('click',function(){
		if($('input[name=checkAll]').is(":checked")){
			...
			$('#monSalesPlanTable tbody tr').addClass('selected');
		}else{
			$('input[name=check]').prop('checked',false);
			$('#monSalesPlanTable tbody tr').removeClass('selected');
		}
	});
	...
	let bizOrderConsumptTable = $('#bizOrderConsumptTable').DataTable({
		...
		scrollY : "63vh",
		ajax : {
			url : '<c:url value="bs/bizOrderConsumptList" />',
			type : 'GET',
			data : {
				'mrpNo' 		: function(){ return mrpNo; },
				'mrpSeq' 		: function(){ return mrpSeq; },
		...
		columns : [
			...
			{ data : 'monPlanQty', className : 'text-right',
				render : function(data,type,row,meta){
					if(data!=null){
			...
			{ //확정수량
				data  : 'ordPredQty', className : 'text-right',
				render : function(data, type, row,meta) {
					if(status=="read"){
						if(data!=null){
		...
		drawCallback : function() {
			...
			//반제품 중 생산수량이 외주인 것은 행 색깔을 주황색으로 변경
			let api = this.api();
		    for ( var i = 0; i < api.data().count(); i++ ) {
		    	prodGubunArray.forEach(function(value){
		    		if(api.row(i).data().prodGubun == value){
				    	$('#'+api.row(i).data().itemSeq).addClass('bg-orange');
					}
		    	})
			}
		}
