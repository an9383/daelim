	...
	<header class="page-title-bar row">
		<nav aria-label="breadcrumb" class="breadcrumb-padding">
			<ol class="breadcrumb">
				...
				<li class="breadcrumb-item active">금형 SHOT 이력</li>
			</ol>
		</nav>
	</header>
	<!-- #main============================================================== -->
		...
		<div class="modal-dialog modal-xl">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title" id="inLabel">금형 SHOT 이력 엑셀조회</h5>
...
<script>
	...
	$(document).attr("title", "금형 SHOT 이력"); 
	
	let stateCdVal = '';
	let equipStrInOutVal = '';
	let equipCd = ' ';
	...
	let equipStrInOutCode = new Array();
	...
	let equipCodeTable = $('#equipCodeTable').DataTable({
		...
		scrollY : '65vh',
		ajax : {
			url : '<c:url value="/bm/equipShotList"/>',
			type : 'GET',
			data : {
				'stateCd' 		: function(){ return stateCdVal; },
				'equipStrInOut' : function(){ return equipStrInOutVal; },
			},
		},
		columns : [
			{//금형관리번호
	...
	let dayTable = $('#dayTable').DataTable({
		...
		scrollY: '60vh',
		ajax : {
			url : '<c:url value="bm/moldDayList"/>',
			type : 'GET',
			data : {
	...
	let monthTable = $('#monthTable').DataTable({
		...
		scrollY: '60vh',
		ajax : {
			url : '<c:url value="bm/moldMonthList"/>',
			type : 'GET',
			data : {
				'equipCd' : function(){ return equipCd; },
	...
	let yearTable = $('#yearTable').DataTable({
		...
		scrollY: '60vh',
		ajax : {
			url : '<c:url value="bm/moldYearList"/>',
			type : 'GET',
			data : {
				'equipCd' : function(){ return equipCd; },
			},
	...
	html1 += '<select class="custom-select mr-2" id="stateCd"></select>';
	html1 += '<label class="input-label-sm">보관위치</label>'; 
	html1 += '<select class="custom-select" id="equipStrInOutOption"></select>';
	...
	selectBoxAppend(equipStrInOutCode, "equipStrInOutOption", "", "1");
	...
	//구분 선택시
	$('#equipStrInOutOption').on('change', function() {
		equipStrInOutVal = $('#equipStrInOutOption').val();
		$('#equipCodeTable').DataTable().ajax.reload(function(){});
    });
