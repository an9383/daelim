...
<div class="app pda_mo_wrap" style="display: flex;flex-direction: column;height: 100%;">
	<header class="pda_mo_hd">
		<a href="<c:url value="tmsc0210"/>" class="float-left"><img class="p-2" src="/resources/assets/images/mo_home_wh.png"></a> <a href="/tmsc0060"><p class="mo_main_title">완제품검사</p></a>
	</header>
	<!--====-sortbox-->
	<div class="mo_sortbox">
				...
				<div class="col-12 row p-1">
					<div class="col-3 mo_sort_title">BarcodeNo</div>
					<div class="form-group input-sub col-9 m-0 p-0">
					...
					<table id="barcodeDtlTable" class="table table-bordered table-td-center">
						<thead class="thead-light">
							<tr>
								...
								<th class="text-center" style="min-width: 60px">생산수량</th>
							</tr>
						</thead>
					</table>
				</div>
...
<script>
	...
	$(document).attr("title","완제품검사 승인처리");
	let menuAuth = 'tmsc0170';
	...
	let barcodeDtlTable = $('#barcodeDtlTable').on( 'error.dt', function ( e, settings, techNote, message ) {
		if(techNote == 7) {
			...
			pageLength : 20,
			ajax : {
				url : '<c:url value="tm/itemWhsAdmRead"/>',
				type : 'GET',
				data : {
			...
			columns : [ 
					...
					{data : 'itemCd',
						render: function(data, type, row, meta) {	
							if(data!=null){
								return '<input type="text" class="form-control" name="itemCd" value="'+data+'" style="max-width:100%;text-align:center;" disabled>';		
							} else{
								return '<input type="text" class="form-control" style="max-width:100%; text-align:center;" name="itemCd" value="'+itemCd+'" disabled>';	
							}		    			
			    		}
		    		},
					{data : 'itemRev',
					...
					{data : 'itemCapa',
		    			render: function(data, type, row, meta) {	
							if(data!=null){
								return '<input type="text" class="form-control" style="max-width:100%; text-align:center;" name="itemCapa" value="'+data+'" disabled>';			
							} else{
								return '<input type="text" class="form-control" style="max-width:100%; text-align:center;" name="itemCapa" value="'+itemCapa+'" disabled>';	
							}		
						}
					},
					{data : 'pairQty',
						render: function(data, type, row, meta) {	
							if(data!=null){
								return '<input type="text" class="form-control" style="max-width:100%; text-align:right;" name="pairQty" value="'+addCommas(data)+'" disabled>';			
							} else{
								return '<input type="text" class="form-control" style="max-width:100%; text-align:right;" name="pairQty" value="'+addCommas(pairQty)+'" disabled>';	
							}				
			    		}
					}, 
			],
			 	...
			 	$.ajax({
	                ...
	                success: function (res) {
	                    ...
	                    if (res.result == 'ok') {
		                    ...
		                    pairQty = addCommas(data.pairQty);
	...
	$('#btnApprove').on('click',function() {
		...
		$('#barcodeDtlTable tbody tr').each(function(index, item){
			...
			rowData.apQty= $(this).find('td input[name=pairQty]').val().replace(/,/g,'');	
			...
			$.ajax({
				...
				success : function(res) {				
					if (res.result == 'ok') {
						...
						toastr.success('완제품검사 승인처리되었습니다.');
