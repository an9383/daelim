	...
	<header class="page-title-bar row">
		<nav aria-label="breadcrumb" class="breadcrumb-padding">
			<ol class="breadcrumb">
				...
				<li class="breadcrumb-item active">자재정보관리(봉제)</li>
			</ol>
		</nav>
	</header>
	<!-- #main============================================================== -->
				...
				<div class="card mb-2" id="formBox">
					...
					<div class="card-body col-sm-12 p-1">
						...
						<div class="card-header card-tab p-0 mb-2">
							...
							<ul class="nav nav-tabs card-header-tabs m-0">
								...
								<li class="nav-item">
									<a class="nav-link disabled d-none" id="tab5Nav" data-toggle="tab" href="#tab5">출고단가이력</a>
								</li>
							</ul>
							<!-- /.nav-tabs -->
						</div>
					...
					<div id="myTabContent" class="tab-content">
							...
							<form id="form">
								<div class="table-responsive">									
									...
									<table class="table table-bordered" id="itemPartInfoTable">
										...
										<tr>
											<th>*품목구분</th>
											<td><select class="custom-select" id="itemGubun"></select></td>
											<th>*공급사</th>
											<td>
												<div class="row">
										...
										<tr>
											<th>재질</th>
											<td><input type="text" class="form-control" id="itemColor" name="itemColor" maxlength="20"/></td>
											<th>상태</th>
										...
										<tr>
											...
											<th>안전재고</th>
											<td><input type="text" class="form-control" id="safetyStock" name="safetyStock" style="text-align:right;" onkeyup="numberFormat(this, 'float')" disabled></td>
										</tr>
										<tr>
											<th>*수입검사여부</th>
										...
										<tr>
											<th>박스 단위</th>
											<td><input type="text" class="form-control" id="boxCount" name="boxCount" style="text-align:right;" onkeyup="numberFormat(this, 'int')"></td>
											<th>소포장 단위</th>
											<td><input type="text" class="form-control" id="smallPackage" name="smallPackage" style="text-align:right;" onkeyup="numberFormat(this, 'int')"></td>
										</tr>
										<tr>
											<th>사급유무</th>
											<td><select class="custom-select" id="supplyType"></select></td>
											<th>*LH/RH</th>
											<td><select class="custom-select" id="lhrhCd"></select></td>
										</tr>
										<tr>
											<th>*입고단가</th>
											<td>
										...
										<tr id="exportDiv" class="d-none">
											<th>출고단가</th>
											<td>
												<input type="text" class="form-control text-right" id="exportCost" name="exportCost" maxlength="30" value="0" onkeyup="numberFormat(this, 'float')">
										...
										<tr><th colspan="4">창고 이동 정보</th></tr>
										<tr>
											<th>사출 품번</th>
											<td>
												<div class="row">
													<div class="input-sub m-0" style="width: 80%;">
														<input type="hidden" class="form-control" id="sachulSeq" name="sachulSeq" disabled>
														<input type="text" class="form-control" id="sachulCd" name="sachulCd" disabled>
														<button type="button" id="btnItemCd" class="btn btn-primary input-sub-search"
															onClick="itemCdModalShow();">
															<span class="oi oi-magnifying-glass"></span>
														</button>
													</div>
													<div class="ml-1" style="width: 5%;">
														<button type="button" id="sachulCdDelete" class="btn closeBtn" onclick="$('#sachulCd').val(''); $('#sachulSeq').val('')">
															<i class="mdi mdi-close"></i>
														</button>
													</div>
												</div>
											...
											<th>수입검사여부</th>
											<td><select class="custom-select" id="bongjeImportYn"></select></td>
										</tr>
										<!--==========/table 내용 추가==========-->
									</table>
								</div>
						...
						<div class="tab-pane fade" id="tab4">
							<div>
								<button type="button" class="btn btn-danger float-right mr-1 mb-1" id="btnImportDel">입고단가삭제</button>
							</div>
							<div class="table-responsive">
						...
						<div class="tab-pane fade" id="tab5">
							...
							<div class="table-responsive">
								<table class="table table-bordered" id="itemInformationTable2">
									<colgroup>
										<col width="5%">
										<col width="11%">
										<col width="12%">
									...
									<thead>
										<tr>
											<th>No</th>
											<th id="exportOrExport" class="text-center">출고단가</th>
											<th id="exportOrExportDate">적용일자</th>
											<th>변경일자</th>
											<th>작성자</th>
											<th>변경사유</th>
										</tr>
...
<script>
	...
	let currentHref = "bmsc0180";
	let currentPage = $('.' + currentHref).attr('id');

	$('#' + currentPage).closest('.has-child', 'li').addClass(
			'has-open has-active');
	$('#' + currentPage).closest('.menu-item').addClass('has-active');
	$(document).attr("title", "자재정보관리(봉제)");
	...
	selectBoxAppend(importYn, "bongjeImportYn", "", "2");
...
// 			$(this).removeClass('selected');
		...
		$.ajax({
			...
			success : function(res) {
				...
				if (res.result == 'ok') {
					...
					$('#sachulSeq').val(data.sachulSeq);
					$('#sachulCd').val(data.sachulCd);
					...
					selectBoxAppend(importYn, "bongjeImportYn", data.bongjeImportYn, "2");
					
					//유상
					if(data.supplyType=="002"){
	...
	/* let previous ='';
	//창고이동_수입검사여부 항목 변경시
	$('#bongjeImportYn').on('focus',function(){
		...
		if(previous=="001"){
			$.ajax({
				url : '<c:url value="qm/excepBongjeImportYnDel"/>',
				type : 'GET',
				data : {
					'itemSeq' : itemSeq
				},
				success : function(res){
					if(res.data>0){
						toastr.warning('현재 수입검사에서 미승인된 항목이 있습니다. 승인 후 변경 바랍니다.');
						$('#bongjeImportYn').val(previous);
					}
				}
			})
		}
	...
	$('#supplyType').on('change',function(){
		//유상
		if($('#supplyType option:selected').val()=="002"){
			$('#exportDiv').removeClass('d-none');
			$('#tab5Nav').removeClass('d-none');
...
// 			$('#itemWorkplace').val('');
	...
	$('#btnSave').on('click',function() {
		if (activeTab == "기본정보") {
			...
			if ($('#importDate').val()=="") {
				toastr.warning('입고단가적용일을 입력해주세요.');
				$('#importDate').focus();
				return false;
			}
			...
			$.ajax({
				...
				data : {
					...
					'safetyStock' : $('#safetyStock').val(),
					'itemColor' : $('#itemColor').val(),
					'supplyType' : $('#supplyType option:selected').val(),
					'sachulSeq' : $('#sachulSeq').val(),
					'bongjeImportYn' : $("#bongjeImportYn option:selected").val(),
				},
				success : function(res) {
					let data = res.data;
					if (res.result == 'ok') {
...
// 						$('#itemInformationTable').DataTable().ajax.reload(function() {});
	...
	let itemInformationTable = $('#itemInformationTable').DataTable({
		dom : "<'row'<'col-sm-12 col-md-6'l><'col-sm-12 col-md-6'f>>"
			+ "<'row'<'col-sm-12'tr>>"
			+ "<'row'<'col-sm-12 col-md-5'B><'col-sm-12 col-md-7 text-right'i>>",
		language : lang_kor,
		destroy : true,
		paging : false,
		searching : false,
	...
	//출고단가이력 테이블 조회
	let itemInformationTable2 = $('#itemInformationTable2').DataTable({
		dom : "<'row'<'col-sm-12 col-md-6'l><'col-sm-12 col-md-6'f>>"
			+ "<'row'<'col-sm-12'tr>>"
			+ "<'row'<'col-sm-12 col-md-5'B><'col-sm-12 col-md-7 text-right'i>>",
		language : lang_kor,
	...
	$('#tab5Nav').on('click', function() {
		$('#itemInformationTable2').DataTable().ajax.reload(function(){});
		setTimeout(function(){
			itemInformationTable2.draw();
			itemInformationTable2.draw();
		}, 300)
	});
	
	$('#itemInformationTable2_length').html('<div class="row"></div>');
	
	let exportCheck = "";
	
	//출고단가이력 테이블 클릭
	$('#itemInformationTable2 tbody').on('click','tr',function() {
		if ($(this).hasClass('selected')) {
			$(this).removeClass('selected');
			exportCheck = "";
			return false;
		} else {
			$('#itemInformationTable2').DataTable().$('tr.selected').removeClass('selected');
			$(this).addClass('selected');
		}
	
		itemCostSeq = itemInformationTable2.row(this).data().itemCostSeq;
	...
	$('#btnExportDel').on('click', function() {
		...
		$.ajax({
			...
			success : function(res) {
				...
				$('#itemInformationTable2').DataTable().ajax.reload();
				$('#itemPartAdmTable').DataTable().ajax.reload(null, false);
			}
		});
	...
	function itemCdModalShow() { 
		...
		$('#itemCdPopUpTable tbody').on('click', 'tr', function() {
			...
			$('#sachulSeq').val(data.itemSeq);
			$('#sachulCd').val(data.itemCd);
	...
	function uiProc(flag) {
		...
		$("#sachulCdDelete").attr("disabled", flag);
		$("#bongjeImportYn").attr("disabled", flag);
