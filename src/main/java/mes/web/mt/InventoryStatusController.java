...
public class InventoryStatusController {
	...
	public String workOrderAdmMain(Locale locale, Model model) throws Exception {
		logger.info("재고현황 메인");
		try {
			Date date = new Date();
			...
			systemCommonCodeVo.setBaseGroupCd("015"); // 자재분류
			List<CommonCodeAdmVo>  systemCommonCodeList = systemCommonCodeService.CommonCodeList(systemCommonCodeVo);
			model.addAttribute("prcssGubun", systemCommonCodeList );
		}catch(Exception e) {
			e.printStackTrace();
		}
